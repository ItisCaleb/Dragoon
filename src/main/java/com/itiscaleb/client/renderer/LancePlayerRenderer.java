package com.itiscaleb.client.renderer;

import com.itiscaleb.client.model.LanceArmorLayer;
import com.itiscaleb.client.model.LanceItemLayer;
...
@OnlyIn(Dist.CLIENT)
public class LancePlayerRenderer extends LivingRenderer<AbstractClientPlayerEntity, PlayerModel<AbstractClientPlayerEntity>> {

    public LancePlayerRenderer(EntityRendererManager renderManager) {
        this(renderManager, false);
    }

    public LanceArmorLayer<?> armorLayer;
    public LanceItemLayer itemLayer;

    public LancePlayerRenderer(EntityRendererManager renderManager, boolean useSmallArms) {
        super(renderManager, new PlayerModel<>(0.0F, useSmallArms), 0.5F);
        armorLayer = new LanceArmorLayer<>(this, new BipedModel(0.5F), new BipedModel(1.0F));
        itemLayer = new LanceItemLayer(this);
    ...
    public void render(AbstractClientPlayerEntity entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        ...
        postRender(entityIn,matrixStackIn,bufferIn, packedLightIn,partialTicks);
    }
    ...
    protected void preRenderCallback(AbstractClientPlayerEntity entitylivingbaseIn, MatrixStack matrixStackIn, float partialTickTime) {
        ...
        float f0 = entitylivingbaseIn.renderYawOffset - entitylivingbaseIn.prevRenderYawOffset;
        float f = entitylivingbaseIn.prevRenderYawOffset + f0 * partialTickTime;
        float offset = f - entitylivingbaseIn.rotationYaw;
        float rotY =  65f - offset;
        matrixStackIn.scale(0.9375F, 0.9375F, 0.9375F);

        matrixStackIn.rotate(Vector3f.YP.rotationDegrees(rotY));

        //hide part that we want to render post
        this.getEntityModel().bipedHead.showModel = false;
        this.getEntityModel().bipedRightArm.showModel = false;
        this.getEntityModel().bipedLeftArm.showModel = false;
    }

    private void postRender(AbstractClientPlayerEntity player, MatrixStack stack, IRenderTypeBuffer buffers, int light, float tick){
        stack.push();
        ...
        float f0 = player.renderYawOffset - player.prevRenderYawOffset;
        float f = player.prevRenderYawOffset + f0 * tick;
        stack.rotate(Vector3f.YP.rotationDegrees(f));
        stack.rotate(Vector3f.YP.rotationDegrees(180f));
        IVertexBuilder builder = buffers.getBuffer(RenderType.getEntitySolid(getEntityTexture(player)));
        float offset = f - player.rotationYaw;

        //render head
        this.getEntityModel().bipedHead.showModel = true;
        this.getEntityModel().bipedHead.render(stack,builder,light,OverlayTexture.NO_OVERLAY);

        //render hand
        this.getEntityModel().bipedRightArm.showModel = true;
        this.getEntityModel().bipedRightArm.rotateAngleX = 0f;
        this.getEntityModel().bipedLeftArm.showModel = true;
        this.getEntityModel().bipedLeftArm.rotateAngleX = 0f;
        stack.push();
        stack.rotate(Vector3f.YP.rotationDegrees(65f-offset));
        rotatePart(this.getEntityModel().bipedRightArm,-0.3f,0,0.1f,stack,builder,light);
        rotatePart(this.getEntityModel().bipedLeftArm,-0.7f,0,0.2f,stack,builder,light);
        stack.pop();

        //render armor
        armorLayer.renderHelmet(stack,buffers,light,player);

        //render item
        stack.push();
        stack.rotate(Vector3f.YP.rotationDegrees(-offset));
        itemLayer.renderMainHand(stack,buffers,light,player);
        stack.pop();
        stack.pop();
    }

    private void rotatePart(ModelRenderer part, float x, float y, float z, MatrixStack stack, IVertexBuilder builder, int light){
        stack.push();
        stack.rotate(Vector3f.XP.rotation(x));
        stack.rotate(Vector3f.YP.rotation(y));
        stack.rotate(Vector3f.ZP.rotation(z));
        part.render(stack,builder,light, OverlayTexture.NO_OVERLAY);
        stack.pop();
    }
